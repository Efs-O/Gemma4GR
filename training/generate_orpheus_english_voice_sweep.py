"""
Orpheus base only (no Moira LoRA): English text + `{voice}:` prefix for each name.
Writes under data/orpheus_only_english/ (override with ORPHEUS_ENGLISH_OUTPUT_DIR).

Forces USE_MOIRA_LORA=0 after loading generate_pairs (its load_dotenv may set Moira on).
"""
...
def main() -> None:
    ...
    # generate_pairs calls load_dotenv(); force Orpheus-only for this run
    os.environ["USE_MOIRA_LORA"] = "0"

    if _gp.use_moira_lora():
        print("[ERROR] Expected USE_MOIRA_LORA=0 (Orpheus only).")
        sys.exit(1)

    rel = os.getenv("ORPHEUS_ENGLISH_OUTPUT_DIR", "data/orpheus_only_english").strip()
    ...
    print("=" * 55)
    print("  Gemma4GR — Orpheus-only English voice sweep")
    print(f"  Voices ({len(voices)}, no filtering): {', '.join(voices)}")
