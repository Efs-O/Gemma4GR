"""
Greek STT benchmark — measures Word Error Rate on held-out clips.
Place 10-20 Greek WAV files + matching .txt in tests/test_clips/
before running.
"""
...
def transcribe(wav_path: Path) -> str:
    """Call llama.cpp multimodal endpoint for STT."""
    import base64
...
def normalize(text: str) -> str:
    """Normalize Greek text for WER comparison."""
...
def run_stt_benchmark(model_label: str) -> dict:
    """Run STT benchmark on test clips. Returns result dict."""
    try:
        import jiwer
    except ImportError:
    ...
    for wav in clips:
        ...
        status = "✓" if wer_clip < 30 else "~" if wer_clip < 60 else "✗"
